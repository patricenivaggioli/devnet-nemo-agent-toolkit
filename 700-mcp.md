...
In this lab, you integrated NeMo Agent Toolkit (NAT) workflows with Model Context Protocol (MCP) servers, both as a server and as a client:

**Expose NAT tools as MCP services**
- Started an MCP server with `nat mcp serve`, which automatically registers NAT functions as MCP tools and publishes them over an HTTP endpoint.
- Observed how NAT converts function schemas into MCP tool interfaces, including support for workflows exposed as chat-style tools.

**Use remote MCP tools from a workflow**
- Configured `function_groups` of type `mcp_client` to connect to MCP servers and import their tools into NAT.
- Used two transports in one workflow: `stdio` (local time server) and `streamable-http` (remote calculator server).

**Orchestrate across MCP tool groups**
- Ran a ReAct workflow that called tools from both MCP groups to solve a multi-step question (compute a product, fetch current time, then compare).

This MCP integration pattern lets you reuse existing tool servers without rewriting them as NAT functions, while also letting you publish NAT workflows and tools to any MCP-compatible client.
