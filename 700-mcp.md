...
Create the `mcp_calculator_agent` workflow

```bash
cd ~/nemo-agent-toolkit/
nat workflow create mcp_calculator_agent
```

## 7.2 Register the tools

```bash
cd ~/nemo-agent-toolkit/
cat > mcp_calculator_agent/src/mcp_calculator_agent/register.py <<'EOF'
...
```bash
cd ~/nemo-agent-toolkit/
cat > mcp_calculator_agent/configs/config.yml <<'EOF'
...
```bash
nat run --config_file=mcp_calculator_agent/configs/config.yml --input "Is the product of 2 * 4 greater than the current hour of the day?"
```
